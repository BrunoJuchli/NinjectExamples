...
using NinjectTest.DefaultAndAlternateBindingConvention.B;
using Xunit;
...
namespace NinjectTest.DefaultAndAlternateBindingConvention
{
    public class Test
    {
        ...
        public void Whoop()
        {
            ...
            kernel.Get<IQueryHandler<int, int>>().Should().BeOfType<QueryHandler>();

            contextMock.Setup(x => x.CustomRouteValue).Returns("AlternativeOne");
            kernel.Get<IQueryHandler<int, int>>().Should().BeOfType<AlternativeOneQueryHandler>();
