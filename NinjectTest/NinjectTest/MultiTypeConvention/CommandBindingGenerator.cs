namespace NinjectTest.MultiTypeConvention
{
    ...
    public class CommandBindingGenerator : IBindingGenerator
    {
        ...
        public IEnumerable<IBindingWhenInNamedWithOrOnSyntax<object>> CreateBindings(Type type, IBindingRoot bindingRoot)
        {
            ...
            binding.WhenInjectedInto(menuItem); // this is a slight hack due to the return type limitation
            yield return binding;
        }

        private static Type FindMatchingMenuItem(Assembly assembly, string commandName)
        {
        ...
        private static string GetCommandName(Type type)
        {
            if (!type.Name.EndsWith(CommandSuffix))
            {
                string message = string.Format(
                    CultureInfo.InvariantCulture,
                    "the command '{0} does not adhere to the convention to end with '{1}'",
                    type.FullName,
                    CommandSuffix);
                throw new ArgumentException(message);
            }

            return type.Name.Substring(0, type.Name.Length - CommandSuffix.Length);
        }
    }
}